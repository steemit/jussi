...
@pytest.mark.parametrize(
    ...
    [("get_block", ('steemd', 'get_block')), ("call", ('steemd', 'call')),
     ("yo.get_block", ('yo', 'get_block')), ('sbds.get_block', ('sbds', 'get_block')),
     ('sbds.call', ('sbds', 'call')), ('sbds.get_block.get_block',('sbds', 'get_block.get_block')),
     ('sbds.steemd.get_block', ('sbds', 'steemd.get_block'))])
def test_parse_namespaced_method(namspaced_method, expected):
    result = jussi.upstream.parse_namespaced_method(namspaced_method)
...
def test_urns(jsonrpc_request, expected):
    result = jussi.upstream.method_urn(jsonrpc_request)
    assert result == expected


def test_urn_pairs(steemd_method_pairs):
    old, new = steemd_method_pairs
    old_urn = jussi.upstream.method_urn(old)
    new_urn = jussi.upstream.method_urn(new)
