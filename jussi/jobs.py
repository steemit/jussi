...
async def get_last_irreversible_block(app=None,
                                      block_interval=3,
                                      reschedule=False):
    logger.debug(
        'get_last_irreversible_block starting up, "last_irreversible_block_num" to %s',
    ...
    if reschedule:
        await app.config.scheduler.spawn(
            get_last_irreversible_block(
                app=app, block_interval=block_interval, reschedule=reschedule))
