...
async def fetch_ws(sanic_http_request: HTTPRequest,
    ...
    urn = method_urn(jsonrpc_request)
    timer = stats.timer('jsonrpc.requests.%s' % urn)
    timer.start()
...
async def dispatch_batch(sanic_http_request: HTTPRequest,
                         jsonrpc_requests: BatchJsonRpcRequest) -> BatchJsonRpcResponse:
    cached_responses = await cache_get_batch(sanic_http_request.app.config.caches, jsonrpc_requests)
    requests = [dispatch_single(sanic_http_request, jsonrpc_request, skip_cacher_get=True) for i,jsonrpc_request in enumerate(jsonrpc_requests) if not cached_responses[i]] # type: List[Awaitable[Any]]
